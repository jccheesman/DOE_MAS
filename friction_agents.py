...
def setup_agents(llm):
    ...
    friction_modeler = Agent(
        ...
        backstory=(
            ...
            "terrain. You specialize in integrating multiple data layers "
            "into composite cost surfaces using WhiteboxTools. Your method: "
            "1) Reclassify slope into friction (flat <2°=1.0, rolling 2-8°=1.4, "
            "mountain >8°=1.75). 2) Combine land cover (Dynamic World) with "
            "permafrost zonation (Obu et al.) using a lookup matrix — e.g. "
            "grass on continuous permafrost=1.88, trees on sporadic=1.87. "
            "3) Take the max of slope friction and LULC-permafrost friction "
            "per pixel. 4) Override with road surface type where roads exist "
            "(paved=1.0, gravel=1.1, dirt=1.6 from AK DOT data). 5) Set "
            "rivers and water to 999 (impassable) for road, navigable (1.0-1.5) "
            "for barge. 6) For planes, use DEM elevation against FAA minimum "
            "safe altitude (2000ft AGL mountainous) — below MSA is impassable, "
            "above gets friction 10.0. 7) Run WhiteboxTools cost_distance from "
            "each source facility to produce accumulated cost surfaces, then "
            "trace least-cost paths via backlink rasters. 8) Sample friction "
            "along each traced path to compute weighted average friction (WAF) "
            "and path length in miles. Your job is to take raw rasters and "
            "produce a single friction number per edge in the graph database."
        ),
        verbose=True,
        llm=llm,
    ...
    friction_task = Task(
        description=(
            ...
            "1. Use the run_friction_computation tool to build composite "
            "friction rasters (friction_road.tif, friction_barge.tif, "
            "friction_sky.tif) from the GEE-exported layers.\n"
            "2. The tool will run WhiteboxTools cost_distance and cost_pathway "
            "for each facility pair in the connects_to edges.\n"
            "3. Once complete, use query_friction_stats to review the results "
            "by region and delivery method.\n"
            "4. Report a summary table of all criteria (slope, land cover, "
            "road type, port access, etc.), conditions, and the corresponding "
            "friction weights used.\n"
            "5. Flag any regions or methods with unexpectedly high or low "
            "friction values."
        ),
        expected_output=(
